                        ...
                        <div class="entity-seals__seals">
                            ...
                            <select-entity v-if="!entity.seals.length" type="seal" openside="down-right" @select="addSeal(entity, $event)">
                                <template #button="{ toggle }">
                                    <div class="entity-seals__seals--seal">
                                        <div class="entity-seals__seals--addSeal" @click="toggle()">
                                            <mc-icon name="add"></mc-icon>
                                        </div>
                                        <span class="seal-label"><?php i::_e('Cadastrar selo') ?></span>
                                    </div>
                                </template>
                            </select-entity>
                        </div>

                        <mc-tag-list v-if="!entity.seals.length" :tags="['<?= i::esc_attr__('Sem selo: nenhuma oportunidade é reconhecida como deste Ente Federado') ?>']" classes="danger__background"></mc-tag-list>

                        <mc-tag-list v-if="unusableSeals(entity).length" :tags="['<?= i::esc_attr__('Selo indisponível: as oportunidades com ele deixam de ser reconhecidas') ?>']" classes="danger__background"></mc-tag-list>
