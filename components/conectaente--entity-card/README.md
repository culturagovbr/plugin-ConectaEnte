...
- **seal-linked** — emitido com o selo como o card o mostra (`id`, `name`, `usable`, `validity`, `files`), que é o que `POST federativeEntitySeal` devolve.
- **seal-unlinked** — emitido depois de `DELETE federativeEntitySeal`.
...
Cada ente tem um selo só: a caixinha `+` aparece apenas no ente sem selo, abre o `conectaente--seal-picker` sobre o catálogo que veio com a página (nenhuma requisição), e o servidor recusa o segundo. Vincular e remover não recarregam a página: o card emite, e a listagem atualiza o card e o catálogo. Os avisos (sem selo; selo na lixeira ou arquivado; selo que ganhou validade depois de vinculado) são badges vermelhos, `mc-tag-list` com `danger__background`.
